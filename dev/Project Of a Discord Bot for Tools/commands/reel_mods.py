...
class ReelModAnalyzer:
    ...
    async def _download_reel(self, url: str, workdir: Path) -> dict[str, Any]:
        ...
        command = [
            ...
            "--print-json", "-f", "bv*+ba/b", "--merge-output-format", "mp4",
            "-o", str(output), url,
        ]
    ...
    async def _transcribe(self, video: Path, workdir: Path) -> str:
        ...
        def transcribe(device: str, compute_type: str) -> str:
            model = WhisperModel(WHISPER_MODEL, device=device, compute_type=compute_type)
            segments, _ = model.transcribe(str(audio), vad_filter=True)
            return " ".join(s.text.strip() for s in segments if s.text.strip())

        try:
            return await asyncio.to_thread(transcribe, "cuda", "float16")
        except Exception:
            try:
                return await asyncio.to_thread(transcribe, "cpu", "int8")
            except Exception:
                return ""
    ...
    async def _extract_frames(self, video: Path, workdir: Path) -> list[Path]:
        ...
        try:
            duration = float(await asyncio.to_thread(
                lambda: subprocess.check_output(duration_cmd, text=True, timeout=20).strip()
            ))
        except Exception:
            duration = 30.0
            ...
            for frame in frames:
                ...
                payload = {
                    ...
                    "prompt": (
                        ...
                        "Identify visible Minecraft mod names, modded item/block/entity names, "
                        "GUI text, subtitles, or unmistakable mod-specific features. "
                        "Do not guess from generic Minecraft content. Return concise clues only."
                    ),
                    "images": [image],
                    "stream": False,
                }
                try:
                    async with session.post(
                        f"{OLLAMA_URL.rstrip('/')}/api/generate", json=payload
                    ) as response:
...
"""
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                if response.status != 200:
                    raise RuntimeError(
                        "Ollama is unavailable. Start Ollama and load the configured models."
                    )
                data = await response.json()

        raw = str(data.get("response", "{}"))
        try:
            mods = json.loads(raw).get("mods", [])
        except json.JSONDecodeError:
    ...
    async def _search_curseforge(self, session: aiohttp.ClientSession, name: str) -> str | None:
        try:
            async with session.get(
                ...
                params={
                    "gameId": "432", "searchFilter": name, "classId": "6", "pageSize": "5"
                },
    ...
    async def reel_mods(self, interaction: discord.Interaction, url: str):
        ...
        if not mods:
            await interaction.followup.send(
                "❌ No Minecraft mod names could be confidently identified."
            )
            return
