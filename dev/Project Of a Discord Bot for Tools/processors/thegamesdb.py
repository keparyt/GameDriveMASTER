...
async def verify_game(name:str)->TGDBGameInfo|None:
    ...
    async with _GAME_LOCK:
        if key in _GAME_CACHE:return _GAME_CACHE[key]
        ranked=await _search_games(query)
        if not ranked:
            _GAME_CACHE[key]=None;return None
        if platform_hint:
            ranked=[r for r in ranked if _platform_matches_hint(r[2],platform_hint)]
            if not ranked:
                log(f"TheGamesDB | no platform match | query={original_query!r} | hint={platform_hint!r}");_GAME_CACHE[key]=None;return None
        best_score=ranked[0][0]
        min_score=0.48 if len(_norm(query).split())<=2 else 0.58
        if best_score<min_score:
            log(f"TheGamesDB | rejected (low title confidence) | query={original_query!r} | best={str(ranked[0][1].get('game_title') or '')!r} | score={best_score:.3f}");_GAME_CACHE[key]=None;return None
        ...
        if len(qtokens)>=3 and len(qtokens&ttokens)/len(qtokens)<0.50:
            log(f"TheGamesDB | rejected (insufficient title token coverage) | query={original_query!r} | title={canonical_name!r}");_GAME_CACHE[key]=None;return None
        same_title_all=[]
        for _,game,platform in ranked:
            title=str(game.get("game_title") or "").strip()
            if _norm(title)==_norm(canonical_name) or _score(canonical_name,title)>=0.90:same_title_all.append((game,platform))
        # The filtered search can omit PC/console variants. Fetching the canonical
        # title again gives us the full platform family for PC prioritization.
        if platform_hint:
        ...
        if not consoles:
            ...
            log(f"TheGamesDB | rejected (no console) | query={original_query!r} | title={canonical_name!r} | hint={platform_hint!r}");return None
        ...
        if not selected_game_id:_GAME_CACHE[key]=None;return None
