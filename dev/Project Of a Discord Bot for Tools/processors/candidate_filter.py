...
# Words that are strong evidence that OCR captured UI, promotion, controls, or
# descriptive copy instead of a title. This is intentionally semantic rather
# than a list of individual bad OCR strings.
NOISE_WORDS = {
    "all", "are", "best", "button", "caption", "captured", "click", "coming",
    "continue", "control", "controls", "co", "coop", "cooperative", "couch",
    "creature", "data", "demo", "description", "descriptions", "download", "drop",
    "early", "evidence", "free", "game", "games", "gameplay", "highly", "ingame",
    "liked", "loading", "menu", "more", "new", "online", "options", "pc", "play",
    "players", "player", "playstation", "press", "price", "promo", "promotional",
    "recommended", "sale", "screen", "select", "sent", "singleplayer", "steam",
    "summer", "terminal", "toggle", "trailer", "try", "tutorial", "ui", "unavailable",
    "watch", "xbox", "youtube",
}

PLATFORM_WORDS = {
    "pc", "playstation", "ps4", "ps5", "ps3", "xbox", "switch", "nintendo", "steam",
    "windows", "mac", "linux",
}

SENTENCE_WORDS = {
    "a", "an", "and", "are", "as", "at", "captured", "for", "from", "has", "have",
    "if", "in", "into", "is", "it", "liked", "new", "of", "on", "that", "the", "these",
    "this", "to", "was", "were", "when", "while", "with", "you", "your",
}

CONTROL_WORDS = {
    "press", "hold", "click", "select", "toggle", "drop", "open", "close", "menu", "options",
    "settings", "inventory", "continue", "loading", "pause", "back", "start", "confirm",
}

GENRE_WORDS = {
    "action", "adventure", "arcade", "battle", "brawler", "fighting", "fps", "horror", "indie",
    "multiplayer", "open", "online", "platformer", "puzzle", "rpg", "roguelike", "sandbox",
    "shooter", "simulation", "singleplayer", "split", "strategy", "survival", "tactical", "world",
    "co", "coop", "cooperative", "couch",
}
...
def rejection_reason(value: str) -> str | None:
    ...
    # Platform lists / storefront targeting are never titles. Require at least
    # two platform references so legitimate names containing one word like
    # "SteamWorld" are not affected.
    if platforms >= 2:
        return "platform list/UI text"

    # Explicit control/UI language is a very strong signal even when the OCR
    # result is short (e.g. "Drop", "Toggle", "Continue").
    if controls and (len(words) <= 3 or noise >= 2):
        return "control/UI text"

    # Marketing/feature copy tends to contain several generic words and verbs.
    if len(words) >= 3 and noise >= max(2, len(words) // 2):
        return "OCR/UI/marketing noise"

    # Sentences with multiple function words are descriptions/captions rather
    # than compact titles. Keep short, distinctive titles unaffected.
    ...
    # One generic word is almost never a useful title candidate, while a
    # multi-word title can legitimately contain generic terms.
...
def _is_partial_duplicate(a: str, b: str) -> bool:
    """Return True when the shorter candidate is clearly contained in the longer."""
    ...
    if len(short) <= 2 and len(long) >= 3 and short.issubset(long):
        return True
    return similarity(short_text, long_text) >= 0.92


def dedupe_candidates(candidates: Iterable[dict], max_items: int = 20) -> list[dict]:
    """Normalize, reject noise, and consolidate exact/near/partial duplicates."""
    accepted: list[dict] = []
    for original in candidates:
        ...
        item.setdefault("_normalized_name", normalize_title(name))

        duplicate_index = None
        for index, existing in enumerate(accepted):
            if _is_partial_duplicate(name, existing["name"]):
        ...
        # Prefer the more complete title when one candidate is a fragment.
    ...
    for item in accepted:
        item.pop("_normalized_name", None)
    return accepted[:max_items]


def dedupe_verified_games(games: Iterable[dict], max_items: int = 20) -> list[dict]:
    """Merge verified records using canonical titles while preserving metadata."""
    output: list[dict] = []
    for game in games:
        ...
        if not name:
            continue
        match_index = next(
            (i for i, existing in enumerate(output) if _is_partial_duplicate(name, existing.get("name", ""))),
            None,
        )
...
def confidence_percent(value) -> int:
    """Normalize either 0..1 or 0..100 confidence into a safe display integer."""
    try:
        score = float(value)
