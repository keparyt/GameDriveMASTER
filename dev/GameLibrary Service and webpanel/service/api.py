...
def _steam_details(app_id):
    ...
    data = item.get("data", {}); movies = data.get("movies") or []; trailer = None
    if movies:
        movie = movies[0]; trailer = (movie.get("mp4") or {}).get("max") or (movie.get("mp4") or {}).get("480") or (movie.get("webm") or {}).get("max") or (movie.get("webm") or {}).get("480")
...
def _merge_match(gd, pn):
    ...
    gd_folder = (gd_root / gd["relative_path"] / "Game") if gd_root else None; gd_exe = None
    try:
        exe_file = Path(f"{gd['last_letter']}:\\") / gd["relative_path"] / "exepath.txt"
        if exe_file.is_file():
            relative = exe_file.read_text(encoding="utf-8-sig").strip().replace("/", "\\")
            if relative and not Path(relative).is_absolute() and ".." not in Path(relative).parts: gd_exe = str((Path(f"{gd['last_letter']}:\\") / gd["relative_path"] / "Game" / relative).resolve())
    ...
    if _norm_name(gd["title"] or gd["name"]) == _norm_name(pn["name"]) and pn_install and gd_root and _path_is_inside(pn_install, str(gd_root)): return True
    return False
...
def create_app(db, metadata=None, config=None, playnite=None):
    ...
    def media_url(playnite_id, kind, value):
        if not value or str(value).startswith(("http://", "https://")): return value
        return f"/api/playnite/media/{urllib.parse.quote(str(playnite_id), safe='')}/{kind}"

    def unified_games(query="", connected_only=False, mode="playlist"):
        gd_rows = [dict(row) for row in db.search(query, connected_only)]; pn_rows = playnite_games(); used=set(); result=[]
        for gd in gd_rows:
            ...
            item = dict(gd); item.update({"unified_id": f"gd:{gd['id']", "source": "gamedrive", "playnite_managed": bool(match), "playnite_id": match["playnite_id"] if match else None, "playtime": match.get("playtime", 0) if match else 0})
            if match:
                for field in ("cover", "hero", "logo", "description", "release_date"):
                    if match.get(field): item[field] = media_url(match["playnite_id"], field, match[field]) if field in ("cover", "hero", "logo") else match[field]
                item["installation_state"], item["launch_source"] = "Installed", "Playnite"
            else: item["installation_state"], item["launch_source"] = ("Installed" if gd["connected"] else "Offline"), "GameDrive"
            result.append(item)
        if mode != "drives":
            for pn in pn_rows:
                if pn["playnite_id"] in used or (query and _norm_name(query) not in _norm_name(pn["name"])): continue
                item = dict(pn); item.update({"id": None, "unified_id": f"pn:{pn['playnite_id']}", "source": "playnite", "playnite_managed": True, "playnite_id": pn["playnite_id"], "title": pn["name"], "connected": True, "last_letter": Path(pn["install_directory"]).drive.rstrip(":") if pn.get("install_directory") else None, "drive_name": "Playnite", "installation_state": "Installed", "launch_source": "Playnite"})
                for field in ("cover", "hero", "logo"): item[field] = media_url(pn["playnite_id"], field, item.get(field))
                result.append(item)
        result.sort(key=lambda x: str(x.get("title") or x.get("name") or "").lower()); return result

    def find_unified(unified_id): return next((item for item in unified_games() if item["unified_id"] == unified_id), None)
    ...
    def network_qr(text: str=Query(...,min_length=1,max_length=500)):
        image=qrcode.make(text); buffer=io.BytesIO(); image.save(buffer,format="PNG"); return Response(content=buffer.getvalue(),media_type="image/png",headers={"Cache-Control":"no-store"})
    ...
    def playnite_fullscreen():
        ...
        try: subprocess.Popen([str(path),"--startfullscreen"],cwd=str(path.parent),creationflags=getattr(subprocess,"CREATE_NO_WINDOW",0)); return {"ok":True,"path":str(path)}
    ...
    def games(q: str=Query("",max_length=200),connected_only: bool=False,mode: str=Query("playlist")):
        mode=mode if mode in ("playlist","drives") else "playlist"; rows=unified_games(q,connected_only,mode)
