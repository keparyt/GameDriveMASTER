...
def _merge_match(gd, pn):
    ...
    root = Path(f"{gd['last_letter']}:\\")
    game_root = root / gd["relative_path"] / "Game"
    pn_install, pn_exe = pn.get("install_directory"), pn.get("executable")
    if pn_install and _norm_path(pn_install) == _norm_path(game_root): return True
    try:
        exe_file = root / gd["relative_path"] / "exepath.txt"
        if exe_file.is_file() and pn_exe:
            rel = exe_file.read_text(encoding="utf-8-sig").strip().replace("/", "\\")
            if rel and not Path(rel).is_absolute() and ".." not in Path(rel).parts:
                if _norm_path(pn_exe) == _norm_path(game_root / rel): return True
    except (OSError, ValueError):
        pass
    if pn_install and (_inside(pn_install, game_root) or _inside(game_root, pn_install)): return True
    return bool(pn_install and _norm_name(gd["title"] or gd["name"]) == _norm_name(pn["name"]) and _inside(pn_install, root))
...
def create_app(db, metadata=None, config=None, playnite=None):
    ...
    def unified_games(query="", connected_only=False, mode="playlist"):
        game_rows = [dict(row) for row in db.search(query, connected_only)]
        ...
        for gd in game_rows:
            ...
            item = dict(gd)
            item.update({"unified_id": f"gd:{gd['id']", "source": "gamedrive", "playnite_managed": bool(match), "playnite_id": match["playnite_id"] if match else None})
            if match:
                for field in ("cover", "hero", "logo"):
                    if match.get(field): item[field] = media_url(match["playnite_id"], field, match[field])
                for field in ("description", "release_date", "playtime"):
    ...
    def find_unified(unified_id):
        return next((item for item in unified_games() if item["unified_id"] == unified_id), None)
    ...
    def playnite_fullscreen():
        ...
        try:
            subprocess.Popen([str(path),"--startfullscreen"],cwd=str(path.parent),creationflags=getattr(subprocess,"CREATE_NO_WINDOW",0)); return {"ok":True,"path":str(path)}
    ...
    def games(q: str=Query("",max_length=200),connected_only: bool=False,mode: str=Query("playlist")):
        rows=unified_games(q,connected_only,mode if mode in ("playlist","drives") else "playlist")
