...
def create_app(db, metadata=None):
    ...
    app.add_middleware(CORSMiddleware, allow_origins=["http://127.0.0.1", "http://localhost"], allow_methods=["GET"], allow_headers=["*"])
    ...
    def drives():
        with db.lock:
            rows = db.conn.execute("SELECT id,uuid,name,description,last_letter,connected,last_seen FROM drives ORDER BY connected DESC,name COLLATE NOCASE").fetchall()
    ...
    def launch_game(game_id: int):
        """Launch GameDrive game from <game>/Game/<exepath.txt content>."""
        row = db.get_game(game_id)
        ...
        game_folder = (drive_root / row["relative_path"]).resolve()
        game_dir = (game_folder / "Game").resolve()
        exe_file = (game_folder / "exepath.txt").resolve()
        try:
            exe_relative = exe_file.read_text(encoding="utf-8-sig").strip().replace("/", "\\")
        except Exception:
            return {"ok": False, "error": "missing_exepath"}
        if not exe_relative or Path(exe_relative).is_absolute() or ".." in Path(exe_relative).parts:
            return {"ok": False, "error": "invalid_exepath"}
