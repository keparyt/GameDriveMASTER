...
class PlayniteBridge:
    ...
    def _ensure_started(self):
        if not self.enabled or not self.playnite_path or not self.playnite_path.is_file():return False
        if self._window():return True
        try:
            # Start Playnite normally and let it initialize its profile/library.
            ...
            self._last_error=str(e);log.warning("Could not start Playnite: %s",e);return False
    def read_games(self,force=False):
        with self._lock:
            if not self.available:
                # Playnite may need to initialize/create its library directory on
                # first launch. Start it in the background once, then retry discovery.
                if self.enabled and not self._startup_attempted and self._ensure_started():
                    deadline=time.monotonic()+5
                    while time.monotonic()<deadline and not self.available:time.sleep(.25)
    ...
    def refresh(self,force=False):
        ...
        try:
            ...
            if not hwnd and self._ensure_started():
                deadline=time.monotonic()+5
                while time.monotonic()<deadline and not self._window():time.sleep(.25)
                hwnd=self._window()
            if hwnd:
                u=ctypes.windll.user32;u.PostMessageW(hwnd,0x0100,0x74,0);u.PostMessageW(hwnd,0x0101,0x74,0);time.sleep(.5)
    ...
    def launch(self,playnite_id):
        ...
        try:
            if not self._window():self._ensure_started();time.sleep(.5)
