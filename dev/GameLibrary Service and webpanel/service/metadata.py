...
class MetadataManager:
    """Resolve game metadata in a background, one-at-a-time worker."""
    ...
    def __init__(self, db, config=None):
        ...
        self._worker = threading.Thread(
            ...
            name="SteamGridDBWorker",
            daemon=True
        )
        self._worker.start()

        log.info(
            "SteamGridDB initialized: enabled=%s auto_lookup=%s cache=%s",
            self.enabled,
            self.auto_lookup,
            self.cache_dir
        )
    ...
    def _worker_loop(self):
        ...
        log.info("SteamGridDB artwork worker stopped")
    ...
    def _download(self, url, destination):
        ...
        log.debug("Downloading artwork: %s -> %s", url, destination)
    ...
    def lookup(self, game_id, game_name):
        if not self.enabled:
            log.debug("SteamGridDB disabled; skipping id=%s name=%r", game_id, game_name)
            return None
            ...
            for field, (category, filename) in assets.items():
                try:
                    ...
                    if url:
                        ...
                        log.debug("No artwork found: %s for id=%s", field, sgdb_id)
