...
class Database:
    ...
    def init(self):
        with self.lock:
            ...
            CREATE INDEX IF NOT EXISTS idx_drives_uuid
                ON drives(uuid);

            CREATE INDEX IF NOT EXISTS idx_metadata_title
                ON metadata(title COLLATE NOCASE);
            """)
    ...
    def upsert_drive(self, uuid, name, description, letter):
        ...
        with self.lock:
            row = self.conn.execute(
                "SELECT id FROM drives WHERE uuid=?",
                (uuid,)
            ).fetchone()
    ...
    def upsert_game(self, drive_id, name, relative_path):
        ...
        with self.lock:
            self.conn.execute(
                ...
                ON CONFLICT(drive_id, relative_path)
                DO UPDATE SET
                    name=excluded.name,
        ...
        with self.lock:
            return self.conn.execute(
                ...
                SELECT
                    ...
                    m.cover,
                    m.release_date,
                    m.description AS game_description
    ...
    def set_metadata(self, game_id, data):
        with self.lock:
            self.conn.execute(
                """
