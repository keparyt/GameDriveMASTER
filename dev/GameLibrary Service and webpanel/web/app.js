...
function patchDrives(ds,hw){const sig=driveSignature(ds,hw);if(sig===currentDrivesSignature)return false;currentDrivesSignature=sig;const usb=hw.filter(d=>String(d.bus||"").toLowerCase()==="usb"),system=hw.filter(d=>String(d.bus||"").toLowerCase()!=="usb");driveCount.textContent=`${usb.filter(d=>!d.offline).length} USB connected Â· ${system.length} system drives`;const wrap=document.createElement("div");wrap.innerHTML=`<div class="storage-section"><div class="storage-section-title"><span>USB DRIVES</span><small>${usb.filter(d=>!d.offline).length} connected</small></div><div class="storage-list">${usb.map(createHardwareCard).join("")||'<span class="muted">No USB drives detected.</span>'}</div></div><div class="storage-section"><div class="storage-section-title"><span>SYSTEM DRIVES</span><small>${system.length} detected</small></div><div class="storage-list">${system.map(createHardwareCard).join("")||'<span class="muted">No system drives detected.</span>'}</div></div>`;drivesBox.replaceChildren(...wrap.children);return true}
async function loadDrives({initial=false}={}){if(drivesBusy)return;drivesBusy=true;const request=++drivesRequest;try{let ds=[],hw=[];const storageResponse=await fetch("/api/system/storage",{cache:"no-store"});if(storageResponse.ok){const value=await storageResponse.json();hw=Array.isArray(value)?value:[]}else{console.debug(`Storage API returned HTTP ${storageResponse.status}`)}try{const driveResponse=await fetch("/api/drives",{cache:"no-store"});if(driveResponse.ok){const value=await driveResponse.json();ds=Array.isArray(value)?value:[]}else{console.debug(`GameDrive API returned HTTP ${driveResponse.status}`)}}catch(e){console.debug("GameDrive API failed",e)}if(request!==drivesRequest)return;const state=driveState(ds);if(previousDriveState!==null&&state!==previousDriveState){const old=new Map(previousDriveState.split("|").filter(Boolean).map(x=>{const p=x.split(":");return[p[0],p[1]==="true"]}));ds.forEach(d=>{const k=String(d.uuid||d.id),before=old.get(k),now=Boolean(d.connected);if(before!==undefined&&before!==now)showDriveNotification(d.name,now)});previousDriveState=state;patchDrives(ds,hw);await loadGames()}else{previousDriveState=state;patchDrives(ds,hw)}if(!ds.length&&!hw.length&&initial)drivesBox.innerHTML='<div class="empty">No storage devices detected.</div>'}catch(e){if(initial){drivesBox.innerHTML='<div class="empty">Unable to load storage devices.</div>';console.error("Drive refresh failed",e)}else console.debug("Drive refresh failed",e)}finally{drivesBusy=false}}
function createCard(g,i){const connected=Boolean(g.connected),cover=g.cover||g.capsule,logo=g.logo,title=g.title||g.name||"Unknown Game",drive=g.drive_name||"Playnite",letter=g.last_letter?`${g.last_letter}:`:"â€”",managed=Boolean(g.playnite_managed),isApp=g.source==="apps",status=g.installation_state||"Installed",gameDriveInstalled=connected&&managed&&!isApp,badge=isApp?"APP":gameDriveInstalled?"INSTALLED Â· GAME DRIVE":managed&&!isApp?"INSTALLED Â· PLAYNITE":status.toUpperCase();return `<article class="card ${connected||managed||isApp?"":"offline-card"}" style="animation-delay:${Math.min(i*18,500)}ms" data-game-id="${escapeHtml(g.unified_id||`gd:${g.id}`)}" tabindex="0" role="button" aria-label="Open ${escapeHtml(title)}"><div class="art">${cover?`<img src="${escapeHtml(cover)}" alt="${escapeHtml(title)}" loading="lazy">`:'<div class="fallback artwork-loading">'+(isApp?'APP':'LOADING ARTWORKâ€¦')+'</div>'}</div><div class="info">${logo?`<img class="logo" src="${escapeHtml(logo)}" alt="" loading="lazy">`:""}<div class="title" title="${escapeHtml(title)}">${escapeHtml(title)}</div><div class="drive">${isApp?"Apps":g.source==="playnite"?"Playnite":drive}${letter!=="â€”"&&!isApp?` Â· ${escapeHtml(letter)}`:""}</div><div class="badge ${connected||managed||isApp?"":"offline"}">${escapeHtml(badge)}</div></div></article>`}
